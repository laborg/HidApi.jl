...
function close(hd::HidDevice)
    hd.handle == C_NULL && error("device isn't open")
    hid_close(hd.handle)
    return hd
end

function read(hd::HidDevice, size = 64, timeout = 2000)
    size > 64 && error("Can't read more than 64 bytes")
    hd.handle == C_NULL && error("device isn't opened")
    ...
    read == -1 && error("error while reading")
    return data
end

function write(hd::HidDevice, data::Vector{UInt8})
